...
function setup() {
  ...
  // }
  clients_canvas = createGraphics(width, height);
  ...
  if (mode === "DE") {
    ...
    for (let i = 0; i < circles_c.length; i++) {
      ...
      for (let j = 0; j < circles_c[i].length; j++) {
        clients_canvas.noStroke();
        clients_canvas.fill(circles_c[i][j].color);
        clients_canvas.ellipse(
          circles_c[i][j].pos.x,
          circles_c[i][j].pos.y,
          circles_c[i][j].r,
          circles_c[i][j].r
